...
function writePassword() {
  ...
  var NumbersSUL = "0123456789!@#$%^&*()ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";
  ...
  var NumbersLU = "0123456789ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz";

  var SpecialU = "!@#$%^&*()ABCDEFGHIJKLMNOPQRSTUVWXYZ";
  var SpecialL = "!@#$%^&*()abcdefghijklmnopqrstuvwxyz";
  var SpecialLU = "!@#$%^&*()abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ";
  ...
  var FirstPrompt = window.confirm("Welcome to the Password Generator. Would you like this password to have lowercase letters?");
  ...
  var LengthPrompt = window.prompt("How long do you want this password to be? Choose a number between 8-128");

  if (!FirstPrompt && !SecondPrompt && !ThirdPrompt && !FourthPrompt){
    window.alert("Sorry, if you don't want any type of text we can't make a password for you. See ya");
    return;
  }
...
if (FirstPrompt && SecondPrompt && ThirdPrompt && FourthPrompt){
  ...
  for (var i = 0; i<= LengthPrompt; i++){
    var Randomer3 = Math.floor(Math.random() * Lowers.length);
    password += Lowers.substring(Randomer3, Randomer3 +1);
}
